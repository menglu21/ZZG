...
def main():
  ...
  if opt.ismc:
    ...
    if opt.year == "2017":
      p = PostProcessor(".", inputFiles(), modules=[countHistogramsModule(),puAutoWeight_2017(),PrefCorr(),muonIDISOSF2017(),muonScaleRes2017(),eleRECOSF2017(),eleIDSF2017(),photonRECOSF2017(),photonIDSF2017(), ZZG2017()], provenance=True,fwkJobReport=True, jsonInput=runsAndLumis(),outputbranchsel="keep_and_drop.txt")
    if opt.year == "2018":
...
# Sequence for data
  if not (opt.ismc):
    ...
    if opt.year == "2017":
      p = PostProcessor(".", inputFiles(), modules=[muonScaleRes2017(),ZZG2017()], provenance=True,fwkJobReport=True, jsonInput=runsAndLumis(),outputbranchsel="keep_and_drop.txt")
    if opt.year == "2018":
